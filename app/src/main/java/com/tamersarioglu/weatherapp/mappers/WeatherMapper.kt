...
class WeatherMapper @Inject constructor() {
    ...
    fun mapToDomain(dto: WeatherResponseDto): WeatherInfo {
        ...
        return WeatherInfo(
            ...
            forecast = dto.data.forecast.map(::mapForecastDay),
            location = mapLocation(dto.data.request.first()),
            climateAverages = dto.data.climateAverages.firstOrNull()?.let(::mapClimateAverages)
        )
    }
    ...
    private fun mapForecastDay(dto: ForecastDto) = ForecastDay(
        ...
        hourlyForecasts = dto.hourly.map(::mapHourlyForecast)
    )
